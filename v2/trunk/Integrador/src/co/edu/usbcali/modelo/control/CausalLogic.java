...
import co.edu.usbcali.dataaccess.dao.ICausalDAO;
...
@Scope("singleton")
@Service("CausalLogic")
public class CausalLogic implements ICausalLogic {
	/**
	...
	/**
	 * Logic injected by Spring that manages TipoCausal entities
	 * 
	 */
	@Autowired
	ITipoCausalLogic logicTipoCausal1;

	@Transactional(readOnly = true)
	public List<Causal> getCausal() throws Exception {
		List<Causal> list = new ArrayList<Causal>();

		try {
			list = causalDAO.findAll();
		} catch (Exception e) {
	...
	public void saveCausal(Causal entity) throws Exception {
		try {
			if (entity.getTipoCausal() == null) {
				throw new ZMessManager().new ForeignException("tipoCausal");
			}
	...
	public void deleteCausal(Causal entity) throws Exception {
		if (entity == null) {
			throw new ZMessManager().new NullEntityExcepcion("Causal");
		}
		...
		try {
			documentos = documentoDAO.findByProperty("causal.idCusa",
					entity.getIdCusa());
			...
			movimientoInventarios = movimientoInventarioDAO.findByProperty(
					"causal.idCusa", entity.getIdCusa());
			...
			visitas = visitaDAO.findByProperty("causal.idCusa",
					entity.getIdCusa());
	...
	public List<CausalDTO> getDataCausal() throws Exception {
		try {
			List<Causal> causal = causalDAO.findAll();

			List<CausalDTO> causalDTO = new ArrayList<CausalDTO>();

			for (Causal causalTmp : causal) {
				CausalDTO causalDTO2 = new CausalDTO();

				causalDTO2.setIdCusa(causalTmp.getIdCusa());
				causalDTO2
						.setCodigo((causalTmp.getCodigo() != null) ? causalTmp
								.getCodigo() : null);
				causalDTO2
						.setDescripcion((causalTmp.getDescripcion() != null) ? causalTmp
								.getDescripcion() : null);
				causalDTO2
						.setEstadoRegistro((causalTmp.getEstadoRegistro() != null) ? causalTmp
								.getEstadoRegistro() : null);
				causalDTO2.setFechaCreacion(causalTmp.getFechaCreacion());
				causalDTO2.setFechaModificacion(causalTmp
						.getFechaModificacion());
				causalDTO2
						.setOperCreador((causalTmp.getOperCreador() != null) ? causalTmp
								.getOperCreador() : null);
				causalDTO2
						.setOperModifica((causalTmp.getOperModifica() != null) ? causalTmp
								.getOperModifica() : null);
				causalDTO2.setIdTcau_TipoCausal((causalTmp.getTipoCausal()
						.getIdTcau() != null) ? causalTmp.getTipoCausal()
						.getIdTcau() : null);
				causalDTO.add(causalDTO2);
			}
	...
	public Causal getCausal(Long idCusa) throws Exception {
		...
		try {
			entity = causalDAO.findById(idCusa);
		} catch (Exception e) {
			throw new ZMessManager().new FindingException("Causal");
		} finally {
		}
	...
	public List<Causal> findPageCausal(String sortColumnName,
			...
			throws Exception {
		List<Causal> entity = null;

		try {
			entity = causalDAO.findPage(sortColumnName, sortAscending,
					startRow, maxResults);
		} catch (Exception e) {
			throw new ZMessManager().new FindingException("Causal Count");
		} finally {
		}
	...
	public List<Causal> findByCriteria(Object[] variables,
		...
		try {
			list = causalDAO.findByCriteria(where);
		} catch (Exception e) {
